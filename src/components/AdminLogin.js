...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${Base_url}admins/admindash/`, formData);

      // Check if the user is a superuser
      if (response.data.user && response.data.token && response.data.user.is_superuser) {
        login(response.data); // Use login function from context to update state and localStorage
        navigate('/admin/dashboard'); // Redirect to admin dashboard after successful login
      } else if (response.data.user && !response.data.user.is_superuser) {
